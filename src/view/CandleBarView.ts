...
export default class CandleBarView extends ChildrenView {
  ...
  private _drawLine3(ctx: CanvasRenderingContext2D, yAxis: Axis): void {
    ...
    for (const item of visibleDataList) {
      _close.lineTo(item.x, yAxis.convertToPixel(item.data.close))
      _high.lineTo(item.x, yAxis.convertToPixel(item.data.high))
      _low.lineTo(item.x, yAxis.convertToPixel(item.data.low))
    }
    ctx.save();
    ctx.strokeStyle = '#888888'
    ctx.stroke(_close)
  ...
  private _drawShade3(ctx: CanvasRenderingContext2D, yAxis: Axis): void {
    ...
    for (const item of visibleDataList) {
      _xs.push(item.x)
      _close.push(yAxis.convertToPixel(item.data.close))
      _high.push(yAxis.convertToPixel(item.data.high))
      _low.push(yAxis.convertToPixel(item.data.low))
    }
